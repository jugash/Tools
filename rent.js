...
db.open(function(err,db) {
	if(!err) {
		...
		db.collection('rentalprices', {strict:true}, function(error, collection) {});
	}
});
...
var findURL = "/property-to-rent/" + town + ".html?minBedrooms=1&radius=1.0&maxDaysSinceAdded=" + days;
...
crawler.on("fetchcomplete", function(queueItem, responseBuffer, response) {
	...
	if(queueItem.path.match(/^\/property-to-rent\/property-[0-9]*.html/i)) {
		...
		{
			var regExp = /([0-9])[\s]*bed[\s]*room[\s]*([\S\s]*)\sto\srent$/i;
	 ...
	 //  			console.log(JSON.stringify(data));
		...
		db.collection('rentalprices', function(error, collection) {
		...
		$('li.moredetails').each(function(i,elem) {
			...
			if(href) {
				...
				var regExp = /([0-9][0-9]?) ([a-zA-z][a-zA-z][a-zA-z]) ([0-9][0-9][0-9]?[0-9]?)/i;
